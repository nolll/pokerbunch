...
namespace Tests.Application.Factories
{
	class CashgameSuiteFactoryTests : TestBase
    {
        ...
        {
            var player = new PlayerInTest(id: 1);
            _players.Add(player);
		...
		private void SetUpTwoGamesWithOneWinningAndOneLosingPlayer()
		{
            ...
            var totalResult1 = new CashgameTotalResultInTest(winningAmount, gameCount, winningTimePlayed);
            var totalResult2 = new CashgameTotalResultInTest(losingAmount, gameCount, losingTimePlayed);
