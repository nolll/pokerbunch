...
const url = computed(() => urls.cashgame.details(props.bunch.id, props.game.id));
const displayDate = computed(() => format.monthDay(props.game.date));
const duration = computed(() => format.duration(props.game.duration));
const formattedAverageBuyin = computed(() => format.currency(props.game.averageBuyin, props.localization));
const formattedTurnover = computed(() => format.currency(props.game.turnover, props.localization));
</script>
