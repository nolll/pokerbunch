...
namespace Tests.Infrastructure.Data.SqlServer
{
    public class SqlServerCashgameStorageTests : StorageTests
    {
        ...
        public void GetGame_CallsStorageWithCorrectSql()
        {
            ...
            const string expectedSql = "SELECT g.GameID, g.Location, g.Status, g.Date, cp.CheckpointID, cp.PlayerID, cp.Type, cp.Stack, cp.Amount, cp.Timestamp FROM game g LEFT JOIN cashgamecheckpoint cp ON g.GameID = cp.GameID WHERE g.GameID = 1";
        ...
        public void GetGames_CallsStorageWithCorrectSql()
        {
            ...
            const string expectedSql = "SELECT g.GameID, g.Location, g.Status, g.Date, cp.CheckpointID, cp.PlayerID, cp.Type, cp.Stack, cp.Amount, cp.Timestamp FROM game g LEFT JOIN cashgamecheckpoint cp ON g.GameID = cp.GameID WHERE g.GameID IN (1,2,3) ORDER BY g.GameID, cp.PlayerID, cp.Timestamp";
