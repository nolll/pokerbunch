namespace tests\AppTests\Cashgame\Listing{
	...
	class CashgameTableModelTests extends UnitTestCase {
		...
		function test_Homegame_IsSet(){
			$model = getModel();
			assertIsA($model.homegame, 'entities\Homegame');
		}

		function test_Cashgames_IsSet(){
			cashgames = getCashgames();
			$model = getModel();
			assertEqual(3, count($model.cashgames));
		}
		...
		function test_Table_WithOneCashgame_OneItemIsCorrectType(){
			...
			assertIsA($model.listItemModels[0], 'app\Cashgame\Listing\CashgameTableItem\CashgameTableItemModel');
			assertEqual(3, count($model.listItemModels));
		}

		function getModel(){
			return new CashgameTableModel(homegame, cashgames);
		}
